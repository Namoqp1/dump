...
Set:AddInput("Input", {
	Title = "Distance Farm",
	Default = config["Distance Farm"] or 0,
	Placeholder = " ",
	Numeric = true,
	Finished = false,
...
Set:AddDropdown("Select Method", {
	...
	Multi = false,
	Default = config["Select Method"] or false,
	Callback = function(v)
		_G.Method = v
	end
})
...
local MultiDropdown = Set:AddDropdown("Select Weapon", {
	...
	Multi = true,
	Default = config["Select Weapon"] or {},
...
local ez = Set:AddDropdown("Select Accessory", {
	...
	Multi = false,
	Default = config["Select Accessory"] or false,
...
Set:AddToggle("Automatic Equip", {
	...
	Default = config["Automatic Equip"] or false, 
...
Skill:AddToggle("Auto Skill All", {
	...
	Default = config["Auto Skill All"] or false,
...
event:AddToggle("Auto Farm Speed Grandma [Fully]", {
	...
	Default = config["Auto Farm Speed Grandma [Fully]"] or false,
...
event:AddToggle("Auto Farm Event", {
	...
	Default = config["Auto Farm Event"] or false,
...
main:AddToggle("Automatic Dungeon", {
	...
	Default = config["Automatic Dungeon"] or false, 
...
event:AddToggle("Auto Farm P'Nuang", {
	...
	Default = config["Auto Farm P'Nuang"] or false,
					...
					for i, v in pairs(workspace.DunMob:GetChildren()) do
						if v and v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v:FindFirstChild("Humanoid").Health > 0 then
							if v.Name ~= "Mahito" then
								...
								repeat task.wait()
									game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.HumanoidRootPart.CFrame * CFrame.new(0,39,0)  * CFrame.Angles(math.rad(-90),0,0)
									Attack()
								until not v or not v:FindFirstChild("Humanoid") or not v:FindFirstChild("HumanoidRootPart") or v:FindFirstChild("Humanoid").Health <= 0 or not _G.Dungeon
							end
						end
...
local ez = store:AddDropdown("Select Ignore Item", {
	...
	Multi = true,
	Default = config["Select Ignore Item"] or {},
...
store:AddToggle("Auto Store", {
	Title = "Auto Store", 
	Default = config["Auto Store"] or false, 
...
spawn(function()
	while wait() do
		pcall(function()
			if _G.AutoStoreFruitone or _G.AutoStoreFruit or _G.Dungeon then
...
Storeall:AddToggle("Auto Store All", {
	...
	Default = config["Auto Store All"] or false, 
	Callback = function(AutoStoreFruit) 
...
spawn(function()
	while task.wait() do
		pcall(function()
			if _G.AutoStoreFruit or _G.Dungeon then
				for i,v in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do 
