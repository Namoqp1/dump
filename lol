...
event:AddButton({
	Title = "Equip Watering vegetables",
	Description = "",
	Callback = function(lol)
		game:GetService("Players").LocalPlayer.Backpack:FindFirstChild("Watering vegetables").Parent = game.Players.LocalPlayer.Character
	end
})
				...
				for i,v in pairs(game:GetService("Players").LocalPlayer.Backpack:GetChildren()) do
					if config and not config["Select Ignore Item"][v.Name] then
						if v:GetAttribute("Type") == "Items" then
							...
							until not v or not config or not config and not config["Select Ignore Item"]
						end
					end
				end
			else
