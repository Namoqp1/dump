...
local Tabs = {
	...
	Trading = Window:AddTab({ Title = "Trading/Party/Economy", Icon = "bomb" }),
...
local Economy = Tabs.Trading:AddSection("Econom") 


local mamung = Economy:AddDropdown("Select Econumy", {
	Title = "Select Econumy",
	Description = "",
	Values = Item,
	Multi = true,
	Default = {},
	Callback = function(ezs)
		_G.SelectEconomy = ezs
	end
})
			...
			pcall(function()
				for i,v in pairs(game:GetService("Players").LocalPlayer.PlayerGui.HUD.For.SellList:GetChildren()) do
					if _G.SelectEconomy[v.Name] then
						for e,z in pairs(game:GetService("Players").LocalPlayer.PlayerGui.HUD.For.SellList:FindFirstChild(v.Name):GetChildren()) do
							if z:IsA("TextLabel") then
								local diamondValue = string.match(z.Text, "(%d+)%s+Diamond")
								print(diamondValue)
								print(v.Name)
								local args = {
									[1] = "fire",
